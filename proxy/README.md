...
![image](https://user-images.githubusercontent.com/11959111/191292511-19d07a53-0368-4a1d-a3aa-337c9b5b8793.png)

`.java` 소스 코드를 컴파일러를 사용해서 `.class`를 만드는 시점에 부가 기능 로직을 추가한다.
이때 AspectJ가 제공하는 컴파일러를 사용, 컴파일 된 `.class` 를 디컴파일 해보면 애스펙트 관련 호출 코드가 들어간다.

AspectJ 컴파일러는 Aspect를 확인해서 해당 클래스가 적용 대상인지 확인하고, 적용 대상인 경우에 부가 기능 로직을 적용한다.

**이처럼 원본 로직에 부가 기능 로직이 추가되는 것을 위빙(Weaving)**이라 한다.
...
![image](https://user-images.githubusercontent.com/11959111/191292456-90a3903d-09e3-47bc-a518-46951a7a5d9b.png)

자바를 실행하면 자바 언어는 `.class`파일을 JVM 내부의 클래스 로더에 보관한다. 이때 중간에서 `.class` 파일을 조작한 다음 JVM에 올릴 수 있다. 자바 언어는 `.class`를 JVM에 저장하기 전에 조작할 수 있는 기능을 제공한다. (java Instrumnetation 검색)
...
![image](https://user-images.githubusercontent.com/11959111/191291610-29eead81-3315-4435-9bcf-293c315854fc.png)

컴파일도 다 끝나고, 클래스 로더에 클래스도 다 올라가서 이미 자바가 실행되고 난 다음.

자바 언어가 제공하는 범위 안에서 부가 기능을 적용해야 한다. 스프링과 같은 컨테이너, 프록시와 DI, 빈 포스트 프로세스 같은 것들을 사용한다. 최종적으로 프록시를 통해 스프링 빈에 부가 기능을 적용할 수 있다. 

프록시를 사용하기 때문에 AOP 기능에 일부 제약이 있으나(생성자 호출 적용x, 메소드에만 적용 가능), 스프링만 있으면 AOP를 쉽게 적용할 수 있다.
